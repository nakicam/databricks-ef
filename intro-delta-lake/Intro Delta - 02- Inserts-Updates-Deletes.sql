...
-- MAGIC %md
-- MAGIC Now when we query that same table, we should see some output!
...
-- MAGIC %md 
-- MAGIC This gives us a simple way to update values. But what if the record is damanged and should be deleted? For this, we need a predicate. A predicate is a statement in SQL that can be evaluated and return back a true or false value. For example, if I say "is this person's name Ada Lovelace?" there are places where that condition is true, and places where that condidion is false.
-- MAGIC 
